...
function _ffmpeg(buffer, args = [], ext = '', ext2 = '') {
  ...
  return new Promise(async(resolve,reject)=>{
    if(!url) throw new TypeError("no url detected")
      ...
      download(url, './' + name + '.png', async function () {
        let a = './' + name + '.png'
        let b = './' + name + '.webp'
        exec(`_ffmpeg -i ${a} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${b}`, async(err) => {
          if (err) reject(err)
          resolve({
            path: b
          })
